...
module LastLLM
  module Providers
    # Define the Openai class directly (for Zeitwerk compatibility)
    class Openai < LastLLM::Provider
      def initialize(config)
        @config = config
        @openai_instance = ::OpenAI.new(config)
      end

      def generate_text(prompt, options = {})
        @openai_instance.generate_text(prompt, options)
      end

      def generate_object(prompt, schema, options = {})
        @openai_instance.generate_object(prompt, schema, options)
      end

      def embeddings(text, options = {})
        @openai_instance.embeddings(text, options)
      end
    end

    # Also keep the OpenAI constant for backward compatibility
    OpenAI = ::OpenAI
  end
end
